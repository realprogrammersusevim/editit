...
impl Editor {
    ...
    pub fn default() -> Self {
        ...
        let document = if let Some(filename) = args.file {
            let doc = Document::open(filename.to_str().unwrap());
            if doc.is_ok() {
                doc.unwrap()
            } else {
                initial_status = format!("Error opening file: {}", filename.to_str().unwrap());
                Document::default()
            }
        } else {
            Document::default()
    ...
    fn search(&mut self) {
            ...
            .unwrap_or(None)
        {
            if let Some(position) = self.document.find(&query[..], &old_position) {
                ...
                self.status_message = StatusMessage::from(format!("Search: \"{query}\" not found"));
            }
        } else {
